...
void sum_mat(int local_mat[], int partner_mat[], int local_n, int sign)
{
    if(sign==1)
    {  
        for(int i = 0; i < local_n; i++)
        {
            for(int j = 0; j < local_n; j++)
            {
                local_mat[i*local_n+j]=  local_mat[i*local_n+j] + partner_mat[i*local_n+j];
            }
        }
    }
    else
    {
        for(int i = 0; i < local_n; i++)
        {
            for(int j = 0; j < local_n; j++)
            {
                local_mat[i*local_n+j]=   partner_mat[i*local_n+j] - local_mat[i*local_n+j];
            }
        }
    }
...
int main(int agrc, char **argv)
{
    ...
    int my_rank, partner_rank, comm_sz, local_mat_size, n, local_n;
    ...
    allocate_local_mat(&partner_mat, local_mat_size);
    ...
    for (int i = 0; i<comm_sz/2;i++)
    {
        if(my_rank%2==i)
        {
            ...
            MPI_Isend(local_mat, local_mat_size,MPI_INT, partner_rank, 0,comm,&request);
        }
        else
        {
            partner_rank = my_rank + (i*2 - 1);
            MPI_Recv(partner_mat, local_mat_size, MPI_INT, partner_rank,0,comm,&recv_status);
        }
    }
    ...
    if (my_rank%2==0)
    {
        sum_mat(local_mat,partner_mat, local_n, 1);
    }
    else
    {

        sum_mat(local_mat,partner_mat, local_n,0);
    }
