...
long long int get_global_sum(int local_val, int my_rank, int comm_sz, MPI_Comm comm)
{
    ...
    for(int gap = 2; gap <= comm_sz; gap*=2)
    {
        if(my_rank% gap < gap/2)
        {
            partner_rank = my_rank + gap/2;
        }
        else
        {
            partner_rank = my_rank - gap/2;
        ...
        MPI_Sendrecv(&local_val, 1, MPI_INT, partner_rank, 0,
                    &partner_val, 1, MPI_INT, partner_rank, 0,
